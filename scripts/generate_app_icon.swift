...
let output = root.appendingPathComponent("AppAssets/AppIcon.icns")
let iconset = URL(fileURLWithPath: NSTemporaryDirectory())
    .appendingPathComponent("MemographAppIcon-\(UUID().uuidString).iconset")
...
guard process.terminationStatus == 0 else {
    throw NSError(domain: "generate_app_icon", code: Int(process.terminationStatus), userInfo: [
...
func writePNG(size: Int, name: String) throws {
    let image = makeImage(size: CGFloat(size))
    ...
    guard let tiff = image.tiffRepresentation,
        ...
        throw NSError(domain: "generate_app_icon", code: 1, userInfo: [
            NSLocalizedDescriptionKey: "Failed to encode PNG \(name)"
        ])
    }
