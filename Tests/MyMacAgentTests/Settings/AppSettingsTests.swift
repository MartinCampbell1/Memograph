...
struct AppSettingsTests {
    ...
    func defaults() {
        ...
        #expect(settings.advisorySidecarMaxConsecutiveFailures == 50)
        #expect(settings.advisorySidecarProviderOrder == ["claude", "gemini", "codex"])
        #expect(settings.advisorySidecarProviderProbeTimeoutSeconds == 20)
        ...
        #expect(settings.advisoryDailyAttentionBudget == 2)
        #expect(settings.advisoryMinGapMinutes == 90)
        #expect(settings.advisoryPerThreadCooldownHours == 12)
        ...
        #expect(settings.advisoryEnabledDomains == [.continuity, .writingExpression])
        #expect(settings.guidanceProfile.enabledDomains == [.continuity, .writingExpression])
